...
export const getCitizensCount = () => async (dispatch: any) => {
    try {
        const count = await citizensAPI.getCitizensCount(dispatch);
...
export const addNewCitizen = (formValues: AddCitizenFormValues) => async (dispatch: any) => {
    try {
        const citizen = await citizensAPI.addNewCitizen(formValues,dispatch);
...
export const getCitizens = (page: number, limit: number) => async (dispatch: any) => {
    try {
        ...
        const count = await citizensAPI.getCitizensCount(dispatch);
        console.log('count getCitizensCount',count)
        const citizens = await citizensAPI.fetchCitizens(page, limit, count,dispatch);
...
export const getCitizenNote = (id: string) => async (dispatch: any) => {
    try {
        const citizenNote = await citizensAPI.fetchNote(id,dispatch);
